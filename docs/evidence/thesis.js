...
front.push(
  ...
  table(
    ...
    [
      ...
      ['8.01', 'Technology selection and rationale', ''],
...
body.push(
  ...
  sub('8.8 Testing and quality tooling'),
  p('JUnit 5 provides the test framework, Spring Boot Test provides context and MockMvc support for driving the HTTP layer without a running server, and JaCoCo measures coverage by instrumenting bytecode during the test run. The choice to measure coverage rather than estimate it is deliberate: an earlier version of the project documentation asserted a coverage figure that was not measured and was wrong by an order of magnitude.'),

  tableCaption('8.01', 'Technology selection and rationale'),
  table(
...
body.push(
  ...
  table(
    ...
    [
      ...
      ['The API gateway is not on the request path', 'It routes the resume paths but has no authentication route, so login cannot traverse it and the client calls the resume service directly. Remedied by adding the route and repointing the client.'],
      ['The circuit breaker is inert', 'The Resilience4j annotation is on a private method invoked by self-invocation, so the Spring AOP proxy never applies it. Remedied by moving the annotation to a proxied method.'],
      ...
      ['No timeout on the model client', 'An unresponsive endpoint can hold a thread indefinitely. Remedied by setting connect and read timeouts.'],
      ['Model failure yields a placeholder', 'A failed call currently produces placeholder content marked COMPLETED rather than FAILED. Remedied by correcting the failure path.'],
      ...
      ['Uneven test coverage', 'Measured coverage is 49.4% of lines on the resume service; the worker and gateway have no test suite.'],
  ...
  ni('Set timeouts on the model clients and repair the circuit breaker so that a slow dependency degrades rather than exhausts the thread pool.'),
  ...
  ni('Route authentication through the gateway so it is genuinely on the request path.'),
